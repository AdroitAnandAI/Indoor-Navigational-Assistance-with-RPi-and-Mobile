...
def build_argparser():
    ...
    ## Can change probability threshold here
...
def isAnnounced(label, x_mid, y_mid):
    ...
    # print("Objects in last frames = ")
    # print(objLastFrames)
    # print(objMidsLastFrames)
    # print ("\n\nCurrent Frame label = {} x_mid = {} y_mid = {} ".format(label, x_mid, y_mid))
    for idx, objs in enumerate(objLastFrames):
        # idy = objs.index(label)
        # print(objs)
        for idy, lbl in enumerate(objs):
            # print(lbl)
            if lbl == label:
                lastMidPt = objMidsLastFrames[idx][idy]
                # print('LastMidPt Found =')
                # # print(objMidsLastFrames[idx])
                # print(lastMidPt)
                # print('Mid Points = ')
                # print((x_mid, y_mid))

                # print("Displacement = " + str(np.abs(lastMidPt[0] - x_mid) + np.abs(lastMidPt[1] - y_mid)))
                if np.abs(lastMidPt[0] - x_mid) + np.abs(lastMidPt[1] - y_mid) < 100:
                    # print ('Already announced this object = ' + label)
                    return True

    # print('This object ' + label + ' is not found')
    return False
...
def announceDetection(frame, frameCount, detections, labels, threshold, output_transform):
    ...
    for detection in detections:
        ...
        if detection.score > threshold:
            ...
            # SAnnounce only if detected objects are expected to be found in the house.
            # This list can be modified to potential objects for better accuracy.
            if class_id not in [0, 2, 28, 32, 36, 43, 44, 46, 47, 48, 49, 50, 61, 62, 63, 64, 66, 67, 68, 69, 70, 72, 73, 74, 75, 76, 80, 83, 87]:
                continue
            ...
            if not isAnnounced(det_label, x_mid, y_mid):
                ...
                announceText = "There is a " + str(det_label) + ' on your '
                if x_mid > frame.shape[1]*3/5:
                    print('Right Side')
                    os.system(
                        'espeak \"' + announceText + ' right side\"')
                elif x_mid < frame.shape[1]*2/5:
                    print('Left Side')
                    os.system(
                        'espeak \"' + announceText + ' left side\"')
                else:
                    os.system(
                        'espeak \"' + announceText + ' front\"')
